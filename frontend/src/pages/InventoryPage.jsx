...
const InventoryPage = () => {
  ...
  const hasWriteAccess =
    ...
    (isOwner ||
      inv?.isPublic ||
      inv?.access?.some((a) => a.userId === user.id));
  ...
  useEffect(() => {
    if (tab === "items") {
      ...
      api
        .get("/items", { params: { inventoryId: id } })
  ...
  const saveFields = async () => {
    ...
    const { data } = await api.put(`/inventories/${id}/fields`, {
      fields: cleaned,
    });
  ...
  const saveCustomId = async () => {
    await api.put(`/inventories/${id}/custom-id-config`, {
  ...
  if (!inv)
    return (
      <div className="container py-5 text-center text-muted">Not found</div>
    );
  ...
  const tabs = [
    ...
    ...(isOwner
      ? [
          ...
          {
            key: "fields",
            label: t("inventory.fields"),
            icon: "bi-layout-text-window",
          },
          {
            key: "access",
            label: t("inventory.access"),
            icon: "bi-shield-lock",
          },
      ...
      <div className="d-flex flex-wrap align-items-start gap-3 mb-4">
        {form.imageUrl && (
          <img
            src={form.imageUrl}
            alt=""
            className="rounded-3 object-fit-cover d-none d-md-block"
        ...
        <div className="flex-grow-1">
          <div className="d-flex align-items-center gap-2 flex-wrap">
            ...
            {inv.category && (
              <span className="badge bg-secondary">{inv.category.name}</span>
            )}
            {inv.isPublic && (
              <span className="badge bg-success">{t("access.publicMode")}</span>
            )}
      ...
      >
        {tabs.map((tb) => (
          <li className="nav-item" key={tb.key}>
            ...
            >
              <i className={`bi ${tb.icon} me-1 d-none d-sm-inline`} />
              {tb.label}
            </button>
          </li>
        ))}
      ...
      {tab === "settings" && isOwner && (
        <div className="row g-3">
              ...
              <div className="card-body">
                ...
                <div className="mb-3">
                  <label className="form-label fw-semibold">
                    ...
                    <span className="text-muted small">(Markdown)</span>
                  </label>
                  <textarea
                    className="form-control"
                    rows={5}
          ...
          <div className="col-md-4">
            {form.description && (
              <div className="card border-0 shadow-sm">
                <div className="card-header fw-semibold bg-transparent">
                  Preview
                </div>
                <div className="card-body">
      ...
      {tab === "fields" && isOwner && (
        <div>
          ...
          <div className="d-flex align-items-center gap-3 mt-3">
            <button className="btn btn-primary" onClick={saveFields}>
      ...
      {tab === "access" && isOwner && (
        <div>
          <div className="form-check form-switch mb-3">
            ...
            />
            <label className="form-check-label fw-semibold" htmlFor="isPublic">
              {t("access.publicMode")}
            </label>
          </div>
            ...
            <>
              <div className="mb-3">
                <UserAutocomplete
                  ...
                  onSelect={(u) => {
                    if (!accessList.find((x) => x.id === u.id))
                      setAccessList((l) => [...l, u]);
                  }}
                />
              </div>
              ...
              <div className="d-flex gap-2 mb-2">
                <button
                  className={`btn btn-sm ${accessSort === "name" ? "btn-primary" : "btn-outline-secondary"}`}
                  onClick={() => setAccessSort("name")}
                >
                  <i className="bi bi-sort-alpha-down me-1" />
                  {t("access.sortByName")}
                </button>
                <button
                  className={`btn btn-sm ${accessSort === "email" ? "btn-primary" : "btn-outline-secondary"}`}
                  onClick={() => setAccessSort("email")}
                >
                  <i className="bi bi-sort-alpha-down me-1" />
                  {t("access.sortByEmail")}
                </button>
              </div>
            ...
            <div className="card border-0 shadow-sm text-center">
              <div className="card-body">
                <div className="display-4 fw-bold text-primary">
                  {stats?._count || inv._count?.items || 0}
                </div>
